...
public class HalfBlindnessLeftLayer<T extends LivingEntity, M extends EntityModel<T>> extends LayerRenderer<T, M> implements IFirstPersonHandLayer {
    public static final ResourceLocation TEXTURE = new ResourceLocation(RotpKingCrimsonAddon.MOD_ID, "textures/entity/half_left.png");

    public HalfBlindnessLeftLayer(IEntityRenderer<T, M> renderer) {
        super(renderer);
    }
    ...
    private ResourceLocation getTexture(EntityModel<?> model, LivingEntity entity) {
        EffectInstance leftBlidness = entity.getEffect(InitEffects.HALF_BLINDNESS_LEFT.get());
        if (leftBlidness != null) {
            return TEXTURE;
        }
        return null;
    }
