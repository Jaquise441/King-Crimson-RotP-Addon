...
public class kingcrimson_timeerase extends StandEntityAction {
    ...
    public void standPerform(World world, StandEntity standEntity, IStandPower userPower, StandEntityTask task) {
        if (!world.isClientSide()) {
            ...
            if (player != null) {
                ...
                //VFX for the King Crimson user only
                InitVFX.startVFXClient(player.getUUID());
            }
        }
    }
    ...
    protected void onTaskStopped(World world, StandEntity standEntity, IStandPower standPower, StandEntityTask task, StandEntityAction newAction) {
        ...
        if (player != null) {
            ...
            if (timeEraseActive) {
                ...
                //VFX for all nearby players, including the King Crimson user
                List<ServerPlayerEntity> nearbyPlayers = world.getEntitiesOfClass(ServerPlayerEntity.class, player.getBoundingBox().inflate(RADIUS));
    ...
    // Fix the first global effect so the only the King Crimson user can see that one.
    // Make a chestplate for the markers that will have a cube model to make the markers more visible.
    // Add a skybox thingy that is only visible for KC user during time erase, probably with another armor stand + chestplate.
    ...
    private static void playSound(PlayerEntity player, SoundEvent sound, boolean forKingCrimsonUserOnly) {
        if (player instanceof ServerPlayerEntity) {
            ...
            if (forKingCrimsonUserOnly) {
                serverPlayer.connection.send(new SPlaySoundEffectPacket(sound, SoundCategory.PLAYERS, player.getX(), player.getY(), player.getZ(), 1.0F, 1.0F));
            } else {
